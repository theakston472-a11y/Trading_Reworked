...
# Purpose:
#
# Search for NEW strategy combinations without repeatedly
# testing the same strategy.
#
# The discovery process:
#
# 1. Loads the feature database.
# 2. Generates combinations of available features.
# 3. Tests each combination across multiple periods.
# 4. Uses a persistent SQLite registry so strategies already
#    tested are skipped on future runs.
# 5. Saves newly discovered candidates to CSV.
# 6. Can be run repeatedly to search different batches.
#
...
# ============================================================
# FEATURES AVAILABLE FOR DISCOVERY
# ============================================================
...
def build_periods(df):
    """
    Build the validation periods used during discovery.

    The periods are deliberately separated so that a strategy
    does not only look good because of one particular section
    of history.
    """
    ...
    # If the database does not contain 2025/2026,
    # fall back to two halves of the available data.
    if not periods:
        midpoint = (
            minimum
            + (maximum - minimum) / 2
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Persistent strategy discovery engine"
        )
    )
...
def prepare_features(df):
    ...
    if not available:
        raise RuntimeError(
            "None of the discovery features "
...
def backtest(
    ...
    for signal_i in indices:
        ...
        # Remove positions which have already
        # closed before this new entry.
        active_exits = [
            exit_i
        ...
        for j in range(
            ...
            # if both SL and TP occur on the
            # same candle, count the SL first.
...
def discover(
    ...
    for condition_count in range(
        ...
        for combo in itertools.combinations(
            ...
            # including failures. This is important:
            # failed strategies should not be tested
