...
Purpose
-------
Take a selected set of strategies and test them across the
entire available market dataset.

This is a separate validation stage from strategy_lab.py.

It:
1. Loads the selected strategies.
...
def load_strategies(path):
    ...
    # IMPORTANT:
    #
    # Some strategy CSV files already contain strategy_id.
    # If it exists, we KEEP it.
    #
    ...
    #
    # ValueError:
    # cannot insert strategy_id, already exists
    #
    # --------------------------------------------------------

    if "strategy_id" not in strategies.columns:
        ...
        strategies["strategy_id"] = pd.to_numeric(
            strategies["strategy_id"],
            errors="coerce"
        )

        if strategies["strategy_id"].isna().any():
            raise RuntimeError(
                "Selected strategy file contains invalid "
                "strategy_id values."
            )
...
def build_portfolio(
    ...
    for trade in candidates:
        ...
        # Remove trades that have already
        # exited before this new trade enters.
...
def strategy_contribution(
    ...
    for strategy_id in sorted(
        ...
        if not accepted_df.empty:
            ...
            accepted_records = accepted.to_dict(
                "records"
            )
