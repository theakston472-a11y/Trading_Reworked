...
def discovery_features_for_direction(direction):
    direction = str(direction).upper().strip()
    directional = BUY_FEATURES if direction == "BUY" else SELL_FEATURES

    # Preserve order while removing duplicates.
    return list(dict.fromkeys(directional + COMMON_FEATURES))
...
DISCOVERY_ENGINE_VERSION = "2026-08-16-v9-directional-fib-gate"
...
def strategy_fingerprint(
    ...
    canonical = canonical_conditions(conditions)

    text = (
        f"{DISCOVERY_ENGINE_VERSION}|"
...
def prepare_features(df, direction):
    requested = discovery_features_for_direction(direction)
    available = []
    ...
    for feature in requested:
        ...
        df[feature] = to_bool_series(df[feature])
    ...
    print("=" * 70)
    print(f"Direction-aware requested features: {len(requested)}")
    ...
    if not available:
        raise RuntimeError("None of the discovery features exist in the database.")
...
def backtest(
    ...
    atrs = df["atr14"].to_numpy(dtype=float, copy=False)

    ratios = _fib_ratios_from_conditions(conditions)
    fib_values = None
    if ratios:
        ratio = max(ratios)
        column = f"fib_{ratio}"
        if column in df.columns:
            fib_values = pd.to_numeric(df[column], errors="coerce").to_numpy(dtype=float, copy=False)

    requested_mode = str(entry_mode).strip().lower()
    if requested_mode == "auto":
        mode = "fib_touch" if fib_values is not None else "next_open"
    else:
        mode = requested_mode
    ...
    results = []
    direction = str(direction).upper().strip()
    wait_bars = max(int(entry_wait_bars), 1)
    max_bars = max(int(max_bars), 1)

    for signal_i_raw in indices:
        signal_i = int(signal_i_raw)

        if mode == "signal_close":
            entry_i = signal_i
            entry = closes[signal_i]
            outcome_start_i = signal_i + 1

        elif mode == "fib_touch":
            if fib_values is None:
                continue

            level = fib_values[signal_i]
            if not np.isfinite(level):
                continue

            first_i = signal_i + 1
            last_i = min(len(df) - 1, signal_i + wait_bars)
            if first_i > last_i:
                continue

            touch = (
                (lows[first_i:last_i + 1] <= level)
                & (highs[first_i:last_i + 1] >= level)
            )
            rel = _first_true(touch)
            if rel is None:
                continue
        ...
        if outcome_start_i >= len(df) or not np.isfinite(entry):
            continue

        # Maintain max-open constraint without scanning closed trades repeatedly.
        active_exits = [x for x in active_exits if x >= entry_i]
        if len(active_exits) >= max_open:
            continue

        atr_value = atrs[signal_i]
        candle_range = abs(highs[signal_i] - lows[signal_i])
        distance = max(candle_range, atr_value) if np.isfinite(atr_value) else candle_range
        ...
        final_i = min(len(df) - 1, outcome_start_i + max_bars - 1)
        h = highs[outcome_start_i:final_i + 1]
        l = lows[outcome_start_i:final_i + 1]
        ...
        stop_rel = _first_true(stop_mask)
        target_rel = _first_true(target_mask)

        # Match existing conservative behaviour: SL wins ties on same bar.
        if stop_rel is not None and (target_rel is None or stop_rel <= target_rel):
            outcome = -1.0
            exit_i = outcome_start_i + stop_rel
        elif target_rel is not None:
            outcome = float(rr)
            exit_i = outcome_start_i + target_rel
        else:
            exit_i = final_i
            movement = (entry - closes[exit_i]) if direction == "SELL" else (closes[exit_i] - entry)
            outcome = float(movement / distance)
    ...
    days = max((last_time - first_time).total_seconds() / 86400.0, 1.0)
    ...
    return {
        ...
        "avg_trades_day": float(len(arr) / days),
    }
...
def discover(
    ...
    for name, start, end in periods:
        ...
        if session_lower not in ("", "all", "none", "nan"):
            session_mask = (
                period_df["session"]
                .astype(str)
                .str.strip()
            ...
            session_mask = np.ones(len(period_df), dtype=bool)

        feature_masks = {
            feature: period_df[feature].to_numpy(dtype=bool, copy=False)
            for feature in features
        }

        # DIRECTIONAL FIB GATE V9
        # Precompute the valid Fib direction once per period.
        fib_direction_col = (
            "fib_bullish"
        ...
        if fib_direction_col not in period_df.columns:
            raise RuntimeError(
                f"Feature database is missing {fib_direction_col}."
            )

        fib_direction_mask = to_bool_series(
            period_df[fib_direction_col]
    ...
    for condition_count in range(
        ...
        for combo in itertools.combinations(
            ...
            for period_name, period_df, session_mask, feature_masks, fib_direction_mask in period_data:
                if period_df.empty:
                    valid = False
                    break

                signal = session_mask.copy()
                for feature in combo:
                    signal &= feature_masks[feature]
                    if not signal.any():
                        break

                # DIRECTIONAL FIB GATE V9
                #
                # BUY + ANY Fib condition:
                #     LOW -> HIGH swing only
                #
                # SELL + ANY Fib condition:
                #     HIGH -> LOW swing only
                if any(
                    "fib" in str(feature).lower()
                    for feature in combo
                ):
                    signal &= fib_direction_mask

                # Guaranteed quick rejection: actual filled trades can never
                # exceed the number of raw setup signals.
                if int(signal.sum()) < min_trades:
                    valid = False
                    break
                ...
                # If this validation period already fails the minimum sample,
                # there is no reason to backtest the remaining period(s).
            ...
            pending_registry += 1
            if pending_registry >= 250:
                connection.commit()
                pending_registry = 0

            if examined % 250 == 0:
                print(
                    f"  Progress: examined={examined:,} | "
                    f"accepted={added:,} | skipped={skipped:,}"
                )
